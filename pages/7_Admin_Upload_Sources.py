...
.upload-hint{background:#EDF3EC;border-radius:10px;padding:1rem 1.2rem;
    font-size:.9rem;color:#3A5040;margin-bottom:1rem;border-left:4px solid #4A7C59;}
</style>
...
<div class="page-header">
    ...
    <div class="page-header-sub">Manage knowledge base sources</div>
</div>
""", unsafe_allow_html=True)
...
if kb_data.get("chunks", 0) > 0:
    ...
    <div class="kb-empty">
        ...
        <div class="kb-detail">Upload your PDF, DOCX, or TXT files below to get started.</div>
    </div>""", unsafe_allow_html=True)

# ── Upload Section ─────────────────────────────────────────────────────────────
st.markdown('<div class="section-card"><div class="section-title">📤 Upload Source Files</div>',
            unsafe_allow_html=True)

st.markdown("""
<div class="upload-hint">
    📌 Upload Swamiji's discourse transcripts, book PDFs, or text files.
...
uploaded_files = st.file_uploader(
    ...
    key="f_uploader",
    help="Select one or more PDF, DOCX, or TXT files"
)
...
upload_clicked = st.button(
    "⬆️ Upload Files to Knowledge Base",
...
if upload_clicked:
    if not uploaded_files:
        st.warning("⚠️ Please select at least one file first using the file chooser above.")
    else:
        meta = dict(
        ...
        for uf in uploaded_files:
            ...
            success_count += 1
            # Try to run ingestion pipeline if available
            try:
                ...
                st.warning(f"⚠️ {uf.name} saved but processing error: {e}")

        if success_count > 0:
            st.info(f"🔨 {success_count} file(s) uploaded. Go to **Admin → Build Knowledge Base** to index them.")

st.markdown('</div>', unsafe_allow_html=True)

# ── YouTube Tab ────────────────────────────────────────────────────────────────
st.markdown('<div class="section-card"><div class="section-title">📹 Add YouTube Transcript</div>',
            unsafe_allow_html=True)
...
yt_lang  = st.selectbox("Transcript Language",
    ["en","hi","kn","te","ta"], key="yt_lang")

st.markdown("<br>", unsafe_allow_html=True)

if st.button("📥 Fetch YouTube Transcript", use_container_width=True):
    if not yt_url:
        ...
        with st.spinner("Fetching transcript from YouTube..."):
            try:
                ...
                st.info("🔨 Go to Admin → Build Knowledge Base to index it.")
            except Exception as e:
                st.error(f"❌ Error: {e}")
